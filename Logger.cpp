...
wstring CLogger::Start()
{
	...
	wstrError = CreateFolder(&wstrPath);
	if (wstrError != L"")
	{
		return wstrError;
	}

	wstrError = CreateFile(&wstrPath);
	if (wstrError != L"")
	{
		return wstrError;
	}
...
wstring CLogger::LogMessage(wstring wstrText)
{
	if (wstrText.empty()|| !m_wofsLogFile.good()) { return L"Invalid pointer\n"; }
